...
export const LoginPage: React.FC = () => {
  ...
  const [email, setEmail] = React.useState('admin@aether.local');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const message = err?.response?.data?.message || err?.message || 'Ошибка входа. Проверьте логин и пароль.';
      setError(message);
    } finally {
      setIsLoading(false);
    }
      ...
      <div className="w-full max-w-md">
        ...
        <div className="bg-surface rounded-card p-8 shadow-xl">
          ...
          <div className="mt-6 p-4 bg-surface-light rounded text-sm text-text-muted">
            <p className="font-medium mb-2">Учетные данные администратора:</p>
            <p>Email: <code className="bg-background px-2 py-0.5 rounded">admin@aether.local</code></p>
